...
// https://github.com/uGuardAdmin/uGuard/issues/405
...
µBlock.whitelistFromString = function(s) {
    ...
    for ( var i = 0; i < lines.length; i++ ) {
        line = lines[i].trim();
        // https://github.com/gorhill/uGuard/issues/171
        // Skip empty lines
        if ( line === '' ) {
            continue;
        }
        ...
        // https://github.com/uGuardAdmin/uGuard/issues/185
        if ( whitelist.hasOwnProperty(key) === false ) {
            whitelist[key] = [];
        }
    ...
    switch ( name ) {
        ...
        case 'experimentalEnabled':
            if ( typeof this.mirrors === 'object' ) {
                // https://github.com/uGuardAdmin/uGuard/issues/540
...
µBlock.toggleFirewallRule = function(details) {
    ...
    // https://github.com/uGuardAdmin/uGuard/issues/731#issuecomment-73937469
    if ( details.persist ) {
        if ( details.action !== 0 ) {
    ...
    // https://github.com/uGuardAdmin/uGuard/issues/420
...
/*
    ...
    https://github.com/gorhill/uGuard/blob/13f2b6b86ff00827650ee2e70ea5f4779845ce4a/src/js/scriptlet-filtering.js#L61

    License is GPL3:
    https://github.com/gorhill/uGuard/blob/master/README.md
*/
